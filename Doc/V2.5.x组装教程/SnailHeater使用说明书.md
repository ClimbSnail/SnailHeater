...
1. 目前固件还未支持自动识别烙铁芯，需要自行在设置中选择__`烙铁类型`__ 方可正常升温。另外，在插接JBC210手柄 __`前`__ ，必须先选择好`烙铁芯类型`，否则会因为温度不对应导致烙铁头高温而损坏烙铁头。T12、JBC245手柄无视此规则。（一车v2.0.x不支持JBC210发热芯）
...
![二车二极管修改D16](二车二极管修改D16.png)
 ...
 __`补充说明：`__ 购买`二车`（v2.5.0）的成品`功率板`和DIY的群友自行修改。购买`主机`且 _`2023年7月12日`_ 之前发货的群友可自行修改，如果不方便，主机可寄回给群主处理。
...
![二车二极管修改D14D7](二车二极管修改D14D7.png)
 ...
 __`补充说明：`__ 购买`二车`（v2.5.0）的成品`功率板`和DIY的群友自行修改。购买`主机`且  __`2023年7月12日`__ 之前发货的群友可自行修改，如果不方便，主机可寄回给群主处理。
