...
本教程对SnailHeater主机接线提供一个统一的接线规范。SnailHeater主机包含5款PCB板，分别为`内嵌板`、`屏幕前面板`、`核心板`、`接线拓展版`、`功率板`。本教程涉及的配件在此在线文档中均有购买连接推荐或者相关描述： https://docs.qq.com/sheet/DQUlDclNBWXhFbUdW

核心板的数据采集来源于功率板，在未接功率板的情况下，数值是无参考意义的。而功率板是的逻辑是由核心板控制的，所以在未接线或核心板不可用的情况下请勿使用AC220供电。
...
<a href="#1. 前面板安装"><font size="3" color="#00f">|-----1. 前面板安装</font></a>
...
<a href="#6. 安装固定功率板"><font size="3" color="#00f">|-----6. 安装固定功率板</font></a>
...
以下推荐使用`SY8205模块`（峰值6A），淘宝5元随意买。但默认此模组出厂为5V，需要自行调整模块上的`R1`和`R2`，将`R1`调整至220k `R2`调整为12K，改完之后理论值为`11.6V`。如下图所示：
...
红线接24V大电源正极、黑线接24V大电源负极、白线接功率板12V标识的接线端子。

***

## <a id="安装">安装</a>

### <a id="1. 前面板安装">1. 前面板安装</a>
前面板有上个开孔，从左到右依次是：GX12-5航空头（烙铁接口）、GX12-4航空头/2MM香蕉头的通用接口（可调电源输出使用）、GX16-8航空头（风枪接口）。

前面板的可调电源口物理上支持两种接口（GX12-4或者2mm香蕉头）。
###### 接口优缺点：
...
![前面板(屏幕板)反面](前面板(屏幕板)反面.jpg)
`R106`、`R107`为预留的调试电阻，不需要焊

![前面板斜视角](前面板斜视角.jpg)
为尽可能美观，故`EC11编码器`（即旋钮）使用倒装的方式安装（将5个引脚用钳子掰180度，一定要轻）
前面板PCB铜柱的连接上`硬件清单中`，规格是`M3*6+1`（1为底部固定引脚的直径）

![触摸面板](触摸面板.jpg)
前面板EC11旋钮下方的裸漏镀锡层（开窗）为触摸开关的按键，需要加一层薄薄的锡，盖上内嵌板后方可导电到上层的内嵌板，实现触摸按键的功能。
...
注：核心板上`R3`圆形封装的电阻是需要焊接热敏电阻，具体型号为`M52-10K-B3380`(本图拍摄时忘记焊上了)
...
3. 品字电源座的`N`线引脚接一根7cm 20AWG的硅胶线并套好热缩管（后续连接功率板需要）。
4. 品字电源座的`L`端接一根短线到`窗船型开关`（注意：事先套好热缩管）
5. `船型开关`的另一端接一根7cm 20AWG的硅胶线并套好热缩管（后续连接功率板需要）。（上图拍照时忘记焊了）
...
### <a id="6. 安装固定功率板">6. 安装固定功率板</a>
![功率板安装](功率板安装.jpg)
1. 先将可控硅的引脚插在功率板上弯折（弯折程度请参考下图）。为了更安全，功率板上的可控硅安装脚距比较大，实际买到的可控硅间距比较小，需要自行拉开一点才可以插进去。
2. 功率板左下角的孔通过一颗`M3*8mm`的螺丝固定到机壳地板，此步骤既是固定功率板，同时也是让机壳连接功率板的地线。

![可控硅绝缘散热结构](可控硅绝缘散热结构.jpg)
一共两组可控硅，推荐型号`BTA08-600`或者`BTA12-600`（尽可能买正品，寿命长），可控硅是带220V市电的，故需要安全可靠的结构。
...
1. 机壳后面板的`接线小板`与`功率板`的fpc规格为`fpc 4pin 15cm同向`，风扇供电使用`XH2.54 双头80mm 同向线`连接。
2. 前面板的`接线拓展板`与`功率板`的fpc规格为`fpc 6pin 6cm同向`，风扇供电使用`XH2.54 双头80mm 同向线`连接。
3. 前置`接线拓展板`的白色地线焊接到`功率板`的小长条的`PE焊点`上

![整机俯视图](整机俯视图.jpg)
1. 连接上`AC-DC 24V`大电源，24V电源为淘宝`24V4A`的廉价电源，峰值8A左右。AC端接功率板的AC端。DC24V输出端接功率板的24V接线座上。
2. 此前准备的`DC-DC 12V`电源模块的红黑线输入端接24V大电源的输出接线座，白线12V接功率板的`12V接线端`（GND旁边的12V标识）。
...
:-: | :-: | :-: | :-: | :-: | :-:
含义 | 识别引脚 | 发热芯正极 | 振动开关正极 | 振动开关/发热芯/热电偶 负极 | 热电偶正极
JBC245手柄架 | 最前端焊点 | 中间段焊点 | 外接热电偶 | 最前端焊点 | 最后端焊点
线序 | 黑 | 红 | 白 | 黑 | 绿
