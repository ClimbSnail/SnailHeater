...
# 打包脚本
# 在项目根目录执行 PyInstaller 命令，具体命令见根目录 README.md
...
EXTERNAL_DATA_ROOT = Path(sys.executable).resolve().parent if getattr(sys, "frozen", False) else PROJECT_ROOT
...
loader.STUBS_DIR = str(PROJECT_ROOT / "stub_flasher")
...
cur_dir = str(PROJECT_ROOT)  # 项目根目录
# 生成的文件目录
...
# 读取配置信息
config_path = EXTERNAL_DATA_ROOT / "SnailHeater_Tool.yaml"
if not config_path.is_file():
    config_path = PROJECT_ROOT / "SnailHeater_Tool.yaml"
cfg_fp = config_path.open("r", encoding="utf-8")
...
if main_appdir_rules:
    main_appdir_rules = str((PROJECT_ROOT / main_appdir_rules).resolve())
if temp_sn_recode_path:
    temp_sn_recode_path = str((PROJECT_ROOT / temp_sn_recode_path).resolve())
    ...
    try:
        ...
        if chip_id == CHIP_ID_S2:
            cmd = ['SnailHeater_WinTool.py', '--port', select_com,
                   ...
                   '0x00001000', str(PROJECT_ROOT / "base_data_new" / ("%s_bootloader_%s.bin" % (chip_id, flash_size_text))),
                   '0x00008000',
                   str(PROJECT_ROOT / "base_data_new" / ("%s_partitions_%s_%d.bin" % (chip_id, flash_size_text, partitions_num))),
                   '0x0002E000', str(PROJECT_ROOT / 'base_data_new' / ('%s_ota_data_initial.bin' % chip_id)),
            ...
            cmd = ['SnailHeater_WinTool.py', '--port', select_com,
                   ...
                   'write_flash',
                   '--flash_size', flash_size_text,
                   '0x00000000', str(PROJECT_ROOT / "base_data_new" / ("%s_bootloader_%s.bin" % (chip_id, flash_size_text))),
                   '0x00008000',
                   str(PROJECT_ROOT / "base_data_new" / ("%s_partitions_%s_%d.bin" % (chip_id, flash_size_text, partitions_num))),
                   '0x0002E000', str(PROJECT_ROOT / 'base_data_new' / ('%s_ota_data_initial.bin' % chip_id)),
                   '0x00030000', os.path.join(main_appdir_rules, firmware_path)
