...
bool initUIObj(uint8_t curIndex)
{
    // 初始化需要显示的信息窗
    bool ret_code = ui_Button[curIndex]->ui_init(ui_PanelMain); // 先初始化才是能进行以下操作
    if (ret_code)
    {
        // Serial0.printf("Init ind = %d, ret_code = %d\n", ind, ret_code);
        lv_obj_t *buttonUI = ui_Button[curIndex]->mainButtonUI;
        lv_group_add_obj(main_btn_group, buttonUI);
        lv_obj_add_event_cb(buttonUI, ui_event_Button, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(buttonUI, ui_Button[curIndex]->pressed_event_cb, LV_EVENT_PRESSED, NULL);
    }
    else
    {
        // Serial0.printf("[Already Init] Init already before, ind = %d\n", ind);
    }
    lv_group_focus_obj(ui_Button[curIndex]->mainButtonUI);
